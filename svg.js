...
class TimeCity{
    ...
    init_svg() {
        var one_sec =1000;var ce = this;
        var da_te = new Date();
        var hrsutc = (this.timeZone/60).toFixed(2);
        var PlusHours = hrsutc!=0 ? ` ${hrsutc>0?`+${hrsutc}`:hrsutc} H`:``;
        this.Citypara.innerText = `${this.CityId} (GMT${PlusHours})`;
        var date = new Date(da_te.getTime() + (da_te.getTimezoneOffset() + this.timeZone)*60000);
        var int_sto = one_sec - (date.getMilliseconds()/1000)*one_sec;
    ...
    readAngles(M12){this.Mode12=M12;
...
let MyCity = new TimeCity(-600,"My City",true);
