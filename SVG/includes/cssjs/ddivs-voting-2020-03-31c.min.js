...
triggerInteractives();var countKatex=(gebi("maincolumn").innerHTML.match(/`/g)||[]).length;var lazyImgs=document.querySelectorAll(".lazy");if(lazyImgs.length>0){for(k=0;k<lazyImgs.length;k++){var lazyImgFromTop=lazyImgs[k].getBoundingClientRect().top;if(lazyImgFromTop<browserHt){lazyImgs[k].src=lazyImgs[k].dataset.src;lazyImgs[k].classList.remove("lazy");}}
lazyImgs=null;}
