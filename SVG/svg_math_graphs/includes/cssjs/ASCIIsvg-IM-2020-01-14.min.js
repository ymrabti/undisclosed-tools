...
var idInc=0;var graphFn=[];var q=[];brdID=boundingDiv.id;var xDomain=brdPropsArr[brdID]["xMax"]-brdPropsArr[brdID]["xMin"];var actualXmin=brdPropsArr[brdID]["actualXmin"];var actualXmax=brdPropsArr[brdID]["actualXmax"];var actualYmin=brdPropsArr[brdID]["actualYmin"];var actualYmax=brdPropsArr[brdID]["actualYmax"];var plotYmin=brdPropsArr[brdID]["plotYmin"];var plotYmax=brdPropsArr[brdID]["plotYmax"];var verySmallNum=1e-15;var pushBeforeYmin=true;if(typeof fun=="string"&&fun.indexOf("y")>-1){if(typeof fun=="string"){eval("g = function(y){ with(Math) return "+mathjs(fun)+"; }");}else if(typeof fun=="object"){eval("f = function(t){ with(Math) return "+mathjs(fun[0])+" }");eval("g = function(t){ with(Math) return "+mathjs(fun[1])+" }");}
...
colorsArr=["#f0f","#050","#f00","#00f","#f55","#55f","#5f5","#f0f","#050","#f00","#00f","#f55","#55f","#5f5","#f0f","#050","#f00","#00f","#f55","#55f","#5f5","#f0f","#050","#f00","#00f","#f55","#55f","#5f5"];var incSub=1;var inc=inc/incSub;var sub=0;var outsideRem=min;function extendPathBefore(inc,g){var incT=inc/30;var ptsToAddArr=[];if(t-inc>min){for(T=t;T>t-inc;T-=incT){gTPrev=g(T-incT);gTPrevPix=boardHeight-(origin[1]+gTPrev*yunitlength);gT=g(T);gTPix=boardHeight-(origin[1]+gT*yunitlength);gTNext=g(T+incT);gTNextPix=boardHeight-(origin[1]+gTNext*yunitlength);slopPrevPixT=(gTPrevPix-gTPix)/(xunitlength*incT);slopNextPixT=(gTPix-gTNextPix)/(xunitlength*incT);if(sign(slopPrevPixT)==sign(slopNextPixT)){ptsToAddArr.unshift([T,gT]);}else{if(slopNextPixT<-50){ptsToAddArr.unshift([T-incT/2,plotYmax]);}else if(slopNextPixT>50){ptsToAddArr.unshift([T-incT/2,plotYmin]);}
break;}}}
return ptsToAddArr;}
function extendPathAfter(inc,g){incT=inc/20;var ptsToAddArr=[];var slopSwingFlagT=false;for(T=t;T<t+inc-incT;T+=incT){gTPrevPrev=g(T-1.1*incT);gTPrev=g(T-incT);gT=g(T);gTNextSmall=g(T+inc/100);gTNext=g(T+incT);gTNextNext=g(T+2*incT);gTPrevPrevPix=boardHeight-(origin[1]+gTPrevPrev*yunitlength);gTPrevPix=boardHeight-(origin[1]+gTPrev*yunitlength);gTPix=boardHeight-(origin[1]+gT*yunitlength);gTNextSmallPix=boardHeight-(origin[1]+gTNextSmall*yunitlength);gTNextPix=boardHeight-(origin[1]+gTNext*yunitlength);slopPrevPrevPixT=(gTPrevPrevPix-gTPrevPix)/(0.1*incT);slopPrevPixT=(gTPrevPix-gTPix)/incT;slopNextSmallPixT=(gTPix-gTNextSmallPix)/(inc/100);slopNextPixT=(gTPix-gTNextPix)/incT;if(Math.abs(slopPrevPrevPixT)<Math.abs(slopPrevPixT)&&(sign(slopPrevPixT)==sign(slopNextPixT))&&(sign(slopPrevPixT)!=sign(slopPrevPix))&&(Math.abs(slopNextPixT)+Math.abs(slopPrevPixT)>100)){slopSwingFlagT=true;}
if(gT>=plotYmin&&gT<=plotYmax){if(gTNext<gt&&sign(slopPrevPix)==sign(slopPrevPixT)&&slopPrevPix<0&&!slopSwingFlagT){ptsToAddArr.push([T,gT]);}
if(gTNext>gt&&sign(slopPrevPix)==sign(slopPrevPixT)&&slopPrevPix>0&&!slopSwingFlagT){ptsToAddArr.push([T,gT]);}
if(gTNext<gt&&sign(slopPrevPix)==sign(slopPrevPixT)&&slopPrevPix<0&&!slopSwingFlagT){ptsToAddArr.push([T,gT]);}}
slopSwingFlagT=false;}
if(ptsToAddArr.length>0){lastYval=ptsToAddArr[ptsToAddArr.length-1][1];}else{lastYval=gT;}
if(lastYval>=plotYmin&&lastYval<=plotYmax&&slopPrevPix>0&&slopNextSmallPixT>10){ptsToAddArr.push([T,plotYmax]);}
if(lastYval>=plotYmin&&lastYval<=plotYmax&&slopPrevPix<-10){ptsToAddArr.push([t,plotYmin]);}
return ptsToAddArr;}
var extendBeforeOnce=true;for(t=min;t<max;t+=inc){if(t!==0&&sign(t)==-1&&sign(t+inc)==1){if(sign(t)==-1){t=-3*verySmallNum;}}
if(t!==0&&sign(t-inc)==-1&&sign(t)==1){if(sign(t)==1){t=3*verySmallNum;}}
gtPrevPrev=g(t-2*inc);gtPrev=g(t-inc);gt=g(t);gtNextSmall=g(t+inc/100);gtNext=g(t+inc);gtNextNext=g(t+2*inc);slopCart=(gtRem-gt)/inc;gtPrevPrevPix=boardHeight-(origin[1]+gtPrevPrev*yunitlength);gtPrevPix=boardHeight-(origin[1]+gtPrev*yunitlength);gtPix=boardHeight-(origin[1]+gt*yunitlength);gtNextSmallPix=boardHeight-(origin[1]+gtNextSmall*yunitlength);gtNextPix=boardHeight-(origin[1]+gtNext*yunitlength);slopPrevPrevPix=(gtPrevPrevPix-gtPrevPix)/inc;slopPrevPix=(gtPrevPix-gtPix)/inc;slopNextSmallPix=(gtPix-gtNextSmallPix)/(inc/100);slopNextPix=(gtPix-gtNextPix)/inc;gtIsNum=(!isNaN(gt)&&Math.abs(gt)!="Infinity")?true:false;gtisLessPlotYmin=(gt<plotYmin)?true:false;gtBetweenMins=(gt>plotYmin&&gt<actualYmin)?true:false;aveMins=0.5*(plotYmin+actualYmin);gtPrevVis=(gtPrev>actualYmin&&gtPrev<actualYmax)?true:false;gtVis=(gt>actualYmin&&gt<actualYmax)?true:false;gtNextVis=(gtNext>actualYmin&&gtNext<actualYmax)?true:false;gtBetweenMaxs=(gt>actualYmax&&gt<plotYmax)?true:false;gtisMorePlotYmax=(gt>plotYmax)?true:false;aveMaxs=0.5*(actualYmax+plotYmax);slopSwingFlag=false;if(Math.abs(slopPrevPrevPix)<Math.abs(slopPrevPix)&&(sign(slopPrevPix)==sign(slopNextSmallPix))&&(sign(slopPrevPix)!=sign(slopNextPix))&&(Math.abs(slopNextPix)+Math.abs(slopPrevPix)>200)){slopSwingFlag=true;}
if(gt=="-Infinity"){pth.push([t,plotYmin]);}
if(gt=="Infinity"){pth.push([t,plotYmax]);}
if(t>0.9&&t<1.1){}
if((gt>=plotYmin&&gt<=plotYmax&&slopSwingFlag==false)){if(t>min&&extendBeforeOnce&&doExtendBefore){ptsToAddArr=extendPathBefore(inc,g);for(i=0;i<ptsToAddArr.length;i++){pth.push(ptsToAddArr[i]);}
extendBeforeOnce=false;}
pth.push([t,gt]);}else{if(pth.length>2){if(gt>=plotYmin&&gt<=plotYmax){ptsToAddArr=extendPathAfter(inc,g);for(i=0;i<ptsToAddArr.length;i++){pth.push(ptsToAddArr[i]);}}else{if(slopPrevPix>0&&gtPrev<=plotYmax){pth.push([t,plotYmax]);}else if(slopPrevPix<0&&gt<=plotYmin){pth.push([t,plotYmin]);}}
if(idInc==0){idSub=id;}else{idSub=id+"-"+idInc;}
path(pth,idSub);plotted=true;pth=[];idInc++;extendBeforeOnce=true;slopSwingFlag=false;}}}}
if(pth.length>2){if(idInc==0){idSub=id;}else{idSub=id+"-"+idInc;}
path(pth,idSub);plotted=true;}
if(!plotted){console.log("There are no points to plot. Are your xMin, xMax and yMin, yMax positioned correctly?");}}
...
node.textContent=strTxt;node.setAttribute("x",(p[0]*xunitlength+origin[0]+dx).toFixed(2));node.setAttribute("y",(boardHeight-p[1]*yunitlength-origin[1]+1*dy).toFixed(2));if(fontsty!=null){node.setAttribute("style",((fontsty!=null&&typeof(fontsty)!="undefined")?fontsty:fontstyle));}
...
var x,y,ldx,ldy,lx,ly,lxp,lyp,pnode,st;dxPix=(dx==null?xunitlength:dx*xunitlength);dyPix=(dy==null?dx:dy*yunitlength);fontsize=Math.max(12,Math.min(dx/2,dy/2,fontsize));ticklength=fontsize/4;if(gdx!==null&&gdx!==0&&gdx>0&&gdy!==null&&gdy!==0&&gdy>0){gdxPix=gdx*xunitlength;gdyPix=gdy*yunitlength;pnode=document.createElementNS('http://www.w3.org/2000/svg',"path");st="";doGrids=1;}else{doGrids=0;}
if(doAxes==1){sAxTix="";removeEle(svgID+"-xAxisVbl");removeEle(svgID+"-yAxisVbl");for(i=0;i<40;i++){removeEle(svgID+"-x-labels-"+i);removeEle(svgID+"-y-labels-"+i);}
pnodeAxTix=document.createElementNS('http://www.w3.org/2000/svg',"path");if(origin[1]>0&&origin[1]<boardHeight){sAxTix+="M0,"+(boardHeight-origin[1]).toFixed(2)+" "+boardWidth.toFixed(2)+","+(boardHeight-origin[1]).toFixed(2);}
ldx=dx/xunitlength;ldy=dy/yunitlength;lx=(xmin>0||xmax<0?xmin:0);ly=(ymin>0||ymax<0?ymin:0);lxp=(ly==0?"below":"above");lyp=(lx==0?"left":"right");var ddx=Math.min(100,Math.max(0,Math.floor(1.1-Math.log(ldx)/Math.log(10))+2));var labelsArrX=[],labelsArrY=[];if(typeof(xAxisVble)=="undefined"){xAxisVble=window.xAxisVble;}
if(typeof(yAxisVble)=="undefined"){yAxisVble=window.yAxisVble;}
txtLabelsClass="intmathItalic";if(origin[0]>=0&&origin[0]<=boardWidth&&origin[1]>=0&&origin[1]<boardHeight){text([xmax+(padding-10)/xunitlength,0],xAxisVble,"above",svgID+"-xAxisVbl","",'KaTeX_Math,"Times New Roman",Times,serif');arrowhead([0,0],[xmax+padding/xunitlength,0],svgID+"-xAxisArr");}
if(showYaxis==1&&origin[0]>=0&&origin[0]<=boardWidth){text([0,ymax+(padding-10)/yunitlength],yAxisVble,"right",svgID+"-yAxisVbl","",'KaTeX_Math,"Times New Roman",Times,serif');arrowhead([0,ymin],[0,ymax+padding/yunitlength],svgID+"-yAxisArr");}
txtLabelsClass="intmath";markerstrokewidth=0.5;stroke="#555";var ddy=Math.max(0,Math.floor(1.1-Math.log(ldy)/Math.log(10))+2);if(ddy<0){console.log("ddy is < 0. Aborting...");return;}
if(showYaxis==1&&origin[0]>0&&origin[0]<boardWidth){sAxTix+=" M"+origin[0].toFixed(2)+",0 "+origin[0].toFixed(2)+","+boardHeight.toFixed(2);}}
var maxDecPlaces=10;if(doGrids==1){for(x=origin[0]+gdxPix;x<boardWidth;x=x+gdxPix){if(x>0){st+=" M"+x.toFixed(2)+",0"+" "+x.toFixed(2)+","+boardHeight.toFixed(2);}else{x+=gdxPix*(Math.floor(-origin[0]/gdxPix)-1);}}}
for(x=origin[0]+dxPix;x<boardWidth;x=x+dxPix){if(x>0){if(doAxes==1){if(origin[1]>0&&origin[1]<boardHeight){sAxTix+=" M"+x.toFixed(2)+","+(boardHeight-origin[1]+ticklength).toFixed(2)+" "+x.toFixed(2)+","+
(boardHeight-origin[1]-ticklength).toFixed(2);}
xCart=(x-padding)/xunitlength+xmin;if(labels!=null&&labels!="none"){if(ddx>8&&ddx<100&&decimalPlaces(xCart)==ddx||decimalPlaces(Number(chopZ(xCart.toFixed(ddx))))>decimalPlaces(xCart)){ddx--;}
ddx=Math.min(12,ddx);if(decimalPlaces(xCart)>3&&chopZ(xCart.toFixed(ddx)).length>maxDecPlaces){maxDecPlacesReached=true;}else{maxDecPlacesReached=false;}
if(Math.abs(xCart)>10000&&chopZ(Math.abs(xCart).toFixed(ddx)).length>maxDecPlaces){maxNumberReached=true;}
labelsArrX.push([[xCart,ly],chopZ(xCart.toFixed(ddx)),lxp]);}}}else{x+=dxPix*(Math.floor(-origin[0]/dxPix)-1);}}
if(doGrids==1){for(x=origin[0]-gdxPix;x>0;x=x-gdxPix){if(x<boardWidth){st+=" M"+x.toFixed(2)+",0"+" "+x.toFixed(2)+","+boardHeight.toFixed(2);}else{x-=gdxPix*(Math.floor((origin[0]-boardWidth)/gdxPix)-1);}}}
for(x=origin[0]-dxPix;x>0;x=x-dxPix){if(x<boardWidth){if(doAxes==1){if(origin[1]>0&&origin[1]<boardHeight){sAxTix+=" M"+x.toFixed(2)+","+(boardHeight-origin[1]+ticklength).toFixed(2)+" "+x.toFixed(2)+","+
(boardHeight-origin[1]-ticklength).toFixed(2);}
xCart=(x-padding)/xunitlength+xmin;if(labels!=null&&labels!="none"){if(ddx>8&&ddx<100&&decimalPlaces(xCart)==ddx||decimalPlaces(Number(chopZ(xCart.toFixed(ddx))))>decimalPlaces(xCart)){ddx--;}
ddx=Math.min(12,ddx);if(decimalPlaces(xCart)>3&&chopZ(xCart.toFixed(ddx)).length>maxDecPlaces){maxDecPlacesReached=true;}else{maxDecPlacesReached=false;}
if(Math.abs(xCart)>10000&&chopZ(Math.abs(xCart).toFixed(ddx)).length>maxDecPlaces){maxNumberReached=true;}
labelsArrX.push([[xCart,ly],chopZ(xCart.toFixed(ddx)),lxp]);}}}else{x-=dxPix*(Math.floor((origin[0]-boardWidth)/dxPix)-1);}}
if(showYaxis==1&&doGrids==1){for(y=boardHeight-origin[1]-gdyPix;y>0.99*padding;y=y-gdyPix){if(y>=0&&y<=boardHeight){st+=" M0,"+y.toFixed(2)+" "+boardWidth+","+y.toFixed(2);}else{y-=gdyPix*(Math.floor(-origin[1]/gdyPix)-1);}}}
if(showYaxis==1){for(y=boardHeight-origin[1]-dyPix;y>0.99*padding;y=y-dyPix){if(y>=0&&y<=boardHeight){if(origin[0]>0&&origin[0]<boardWidth){sAxTix+=" M"+(origin[0]+ticklength).toFixed(2)+","+y.toFixed(2)+" "+(origin[0]-ticklength).toFixed(2)+","+y.toFixed(2);}
yCart=ymax+(padding-y)/yunitlength;if(labels!=null&&labels!="none"){if(ddy>8&&ddy<100&&decimalPlaces(yCart)==ddy||decimalPlaces(Number(chopZ(yCart.toFixed(ddy))))>decimalPlaces(yCart)){ddy--;}
ddy=Math.min(12,ddy);if(decimalPlaces(yCart)>3&&chopZ(yCart.toFixed(ddy)).length>maxDecPlaces){maxDecPlacesReached=true;}else{maxDecPlacesReached=false;}
if(Math.abs(yCart)>10000&&chopZ(Math.abs(yCart).toFixed(ddy)).length>maxDecPlaces){maxNumberReached=true;}
labelsArrY.push([[lx,yCart],chopZ(yCart.toFixed(ddy)),lyp]);}}else{y-=dyPix*(Math.floor(-origin[1]/dyPix)-1);}}}
if(showYaxis==1&&doGrids==1){for(y=boardHeight-origin[1]+gdyPix;y<boardHeight-0.99*padding;y=y+gdyPix){if(y>=0&&y<=boardHeight){st+=" M0,"+y.toFixed(2)+" "+boardWidth+","+y.toFixed(2);}else{y+=gdyPix*Math.floor((origin[1]-boardHeight)/gdyPix);}}}
if(showYaxis==1){for(y=boardHeight-origin[1]+dyPix;y<boardHeight-0.99*padding;y=y+dyPix){if(y>=0&&y<=boardHeight){if(origin[0]>0&&origin[0]<boardWidth){sAxTix+=" M"+(origin[0]+ticklength).toFixed(2)+","+y.toFixed(2)+" "+(origin[0]-ticklength).toFixed(2)+","+y.toFixed(2);}
yCart=ymax+(padding-y)/yunitlength;if(labels!=null&&labels!="none"){if(ddy>8&&ddy<100&&decimalPlaces(yCart)==ddy||decimalPlaces(Number(chopZ(yCart.toFixed(ddy))))>decimalPlaces(yCart)){ddy--;}
ddy=Math.min(12,ddy);if(decimalPlaces(yCart)>3&&chopZ(yCart.toFixed(ddy)).length>maxDecPlaces){maxDecPlacesReached=true;}else{maxDecPlacesReached=false;}
if(Math.abs(yCart)>10000&&chopZ(Math.abs(yCart).toFixed(ddy)).length>maxDecPlaces){maxNumberReached=true;}
labelsArrY.push([[lx,yCart],chopZ(yCart.toFixed(ddy)),lyp]);}}else{y+=dyPix*Math.floor((origin[1]-boardHeight)/dyPix);}}}
if(doGrids==1){pnode.setAttribute("d",st);pnode.setAttribute("stroke-width",1);pnode.setAttribute("shape-rendering","crispEdges");pnode.setAttribute("stroke",gridstroke);pnode.setAttribute("fill",fill);pnode.setAttribute("id",svgID+"-grids");theSVG.appendChild(pnode);}
if(doAxes==1){pnodeAxTix.setAttribute("d",sAxTix);pnodeAxTix.setAttribute("stroke-width",1);pnodeAxTix.setAttribute("shape-rendering","crispEdges");pnodeAxTix.setAttribute("stroke",axesstroke);pnodeAxTix.setAttribute("fill",fill);pnodeAxTix.setAttribute("id",svgID+"-AxTix");theSVG.appendChild(pnodeAxTix);var widthMax=10;for(i=0;i<labelsArrX.length;i++){text(labelsArrX[i][0],labelsArrX[i][1],labelsArrX[i][2],svgID+"-x-labels-"+i);widthMax=Math.max(widthMax,gebi(svgID+"-x-labels-"+i).getClientRects()[0].width);var xLabelsCumWidth=labelsArrX.length*widthMax;if(xLabelsCumWidth>boardWidth){overSub=Math.ceil(xLabelsCumWidth/boardWidth);if(i%overSub!=0){gebi(svgID+"-x-labels-"+i).style.visibility="hidden";}}}
for(i=0;i<labelsArrY.length;i++){text(labelsArrY[i][0],labelsArrY[i][1],labelsArrY[i][2],svgID+"-y-labels-"+i);}}}
